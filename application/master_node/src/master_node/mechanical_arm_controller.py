...
class MechanicalArmController:
    ...
    def __init__(self, nx_host="192.168.2.103", nx_port=8888):
        """
        初始化机械臂控制器
        ...
        print("机械臂控制器已初始化 - 目标: {}:{}".format(nx_host, nx_port))
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    # 检查是否包含图片相关字段
                    if 'type' in header_json and header_json['type'] in ['result', 'raw', 'rc']:
    ...
    def _is_key_value_header(self, header_str):
        ...
        try:
            # 检查是否包含图片相关字段
            if 'type:' in header_str and ('raw' in header_str or 'result' in header_str or 'rc' in header_str):
    ...
    def _parse_header(self, header_str):
        ...
        try:
            ...
            # 检查是否包含必要的字段
            if 'type' in header_info and header_info['type'] in ['result', 'raw', 'rc']:
    ...
    def _is_json_image_data(self, data):
        ...
        try:
            ...
            try:
                ...
                # 检查是否包含图片相关字段
                if 'type' in json_data and json_data['type'] in ['raw', 'result', 'rc','error_rc']:
    ...
    def _process_json_image_data(self, data):
        ...
        try:
            ...
            # 检查是否有必要的字段
            if 'type' in image_info and image_info['type'] in ['result', 'raw', 'rc','error_rc']:
    ...
    def _is_split_image_packet_header(self, data):
        ...
        try:
            ...
            try:
                ...
                # 检查是否包含图片相关字段
                if 'type' in header_json and header_json['type'] in ['result', 'raw', 'rc', 'error_rc']:
                    if 'file_size' in header_json:
                        file_size = header_json.get('file_size', 0)
                        # 检查文件大小是否合理
                        if 0 < file_size < 50 * 1024 * 1024:  # 0到50MB之间
    ...
    def _register_default_handlers(self):
        ...
        self.message_handlers.update({
            ...
            'rc': self._handle_image_message,      # 处理rc类型图片消息（红外图片）
            'error_rc': self._handle_image_message # 处理error_rc类型图片消息（高温异常红外图片）
        })
    ...
    def _handle_status_message(self, message):
        ...
        if status_value == 'done':
            print("[MECHANICAL_ARM] 收到机械臂完成信号: {}".format(message))
            self._handle_mechanical_arm_completion(message)
        else:
            print("[STATUS] 非完成状态: {}".format(status_value))
    ...
    def _handle_image_message(self, message):
        """
        处理图片消息（type为result、raw、rc或error_rc）
        ...
        try:
            ...
            # 获取消息类型（result、raw、rc或error_rc）
            msg_type = message.get('type', 'unknown')
            print("[IMAGE] 图片类型: {}".format(msg_type))
            
            # 如果是error_rc类型，检查高温异常条件
            if msg_type == 'error_rc':
                # 检查消息中是否包含高温异常相关的字段
                # 可能的字段：temperature, temp, high_temperature, temperature_abnormal等
                header = message.get('header', {})
                temperature = header.get('temperature') or header.get('temp') or header.get('high_temperature')
                temperature_abnormal = header.get('temperature_abnormal', False)
                
                # 检查是否有高温异常标志
                if temperature is not None:
                    # 如果温度超过阈值（例如50度），认为是高温异常
                    try:
                        temp_value = float(temperature)
                        if temp_value > 50.0:  # 高温阈值，可根据实际需求调整
                            print("[IMAGE] 检测到高温异常: 温度 = {}°C".format(temp_value))
                        else:
                            print("[IMAGE] 温度正常: 温度 = {}°C".format(temp_value))
                    except (ValueError, TypeError):
                        print("[IMAGE] 无法解析温度值: {}".format(temperature))
                
                if temperature_abnormal:
                    print("[IMAGE] 检测到高温异常标志: temperature_abnormal = {}".format(temperature_abnormal))
                
                # 即使没有明确的高温异常标志，error_rc类型本身也代表异常情况
                print("[IMAGE] error_rc类型图片，表示存在异常情况（可能包含高温异常）")
    ...
    def _handle_mechanical_arm_completion(self, message):
        ...
        Args:
            message: 完成信号消息，包含status="done"等信息
        """
        try:
            print("[MECHANICAL_ARM] 处理机械臂完成信号: {}".format(message))
            
            with self.completion_condition:
                self.mechanical_arm_completed = True
                print("[MECHANICAL_ARM] 设置机械臂完成标志为True")
                # 通知所有等待的线程
                self.completion_condition.notify_all()
                print("[MECHANICAL_ARM] 已通知所有等待线程")
                print("[MECHANICAL_ARM] 机械臂动作完成，任务管理器应该继续执行")
    ...
    def _send_message(self, message):
        ...
        try:
            ...
            # 直接发送JSON字符串，不添加长度前缀
            with self.connection_lock:
                bytes_sent = self.tcp_socket.sendall(message_bytes)
            
            print("[SEND] 消息发送成功: {}".format(message))
            print("[DEBUG] 发送的字节数: {}, 原始JSON: {}".format(len(message_bytes), message_str))
            return True
    ...
    def send_navigation_point_reached(self, map_id, point_id, camera_type='', label='', fill_light_enabled=False):
        """
        发送机器人到达导航点的消息
        ...
        Args:
            map_id: 地图ID
            point_id: 导航点ID
            camera_type: 拍摄类型，'infrared'表示红外拍摄，其他或空表示可见光拍摄
            label: 机械臂点标签（用于选择特定机械臂点，多个点用逗号分隔），空字符串表示执行所有机械臂点
            fill_light_enabled: 补光灯开关，True表示开启，False表示关闭，仅在可见光拍摄时有效
            
        Returns:
            bool: 发送是否成功
        """
        print("=== 机械臂控制器发送消息 - 开始 ===")
        print("接收参数 - map_id: {}, point_id: {}, camera_type: {}, label: {}, fill_light_enabled: {}".format(map_id, point_id, camera_type, label, fill_light_enabled))
        ...
        # 如果是红外拍摄，添加rc="1"字段（字符串类型）
        if camera_type == 'infrared':
            message['rc'] = '1'
            print("检测到红外拍摄，添加rc='1'字段（字符串类型）")
        else:
            print("可见光拍摄，不添加rc字段")
            # 如果是可见光拍摄且补光灯开启，添加led="1"字段（字符串类型）
            if fill_light_enabled:
                message['led'] = '1'
                print("检测到可见光拍摄且补光灯开启，添加led='1'字段（字符串类型）")
            else:
                print("可见光拍摄，补光灯关闭，不添加led字段")
        
        # 如果有label字段，添加到消息中
        if label and label.strip():
            message['label'] = str(label)
            print("检测到机械臂点标签，添加label字段: {}".format(label))
        else:
            print("未指定机械臂点标签，将执行所有机械臂点")
        
        print("构造的消息: {}".format(message))
        print("发送给NX: mapid={}, poseid={}, camera_type={}, label={}, fill_light_enabled={}".format(map_id, point_id, camera_type, label if label else '所有机械臂点', fill_light_enabled))
        
        result = self._send_message(message)
        print("发送结果: {}".format(result))
        return result
    
    def send_mechanical_arm_command(self, command, parameters=None):
        """
        发送机械臂控制命令
    ...
    def execute_navigation_point_action(self, map_id, point_id, camera_type='', label='', fill_light_enabled=False, wait_for_completion=True, timeout=200):
        """
        执行导航点对应的机械臂动作
        ...
        try:
            ...
            print("拍摄类型: {}".format(camera_type if camera_type else '可见光拍摄（默认）'))
            print("机械臂点标签: {}".format(label if label else '所有机械臂点'))
            print("补光灯开关: {}".format('开启' if fill_light_enabled else '关闭'))
            ...
            success = self.send_navigation_point_reached(map_id, point_id, camera_type, label, fill_light_enabled)
    ...
    def _wait_for_completion(self, timeout=200):
        ...
        try:
            ...
            with self.completion_condition:
                # 等待完成信号，带超时
                result = self.completion_condition.wait(timeout)
                
                if result and self.mechanical_arm_completed:
                    print("[MECHANICAL_ARM] 收到完成信号，机械臂动作完成")
                    return True
                else:
