...
launch_file="drobot_task_manager.launch"
find_pid_key="drobot_task_manager"

function look()
{
    look_proc ${find_pid_key}
    drobot_task_manager=$?
    return $drobot_task_manager
}
...
function launch()
{
    ...
    if [[ ${DROBOT_RECENT_PROC_STOPED} == 1 ]]; then
        ...
        roslaunch ${DROBOT_LAUNCH_DIR}/${launch_file} &> ${DROBOT_LOG_DIR}/drobot_task.log &
    else
