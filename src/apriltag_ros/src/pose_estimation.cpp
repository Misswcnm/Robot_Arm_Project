...
homography(apriltag_detection_t* const detection, const std::array<double, 4>& intr,
           const std::vector<double>&, double tagsize)
{
...
geometry_msgs::msg::Transform
pnp(apriltag_detection_t* const detection, const std::array<double, 4>& intr,
    const std::vector<double>& distortion, double tagsize)
{
    ...
    const cv::Mat distortionCoefficients(distortion);
    cv::Mat rvec, tvec;
    cv::solvePnP(
        objectPoints, imagePoints, cameraMatrix, distortionCoefficients,
        rvec, tvec);
